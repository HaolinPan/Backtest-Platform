...
class BacktestEngine():
    ...
    def setDatabase(self,database,read=False):
        self.database = database
        freq = self.strategy.freq      
        for asset in self.assetList:
            data = database[asset].resample(freq).ohlc().dropna()
    ...
    def plot(self):
        ...
        ax1.grid(axis='y')
        time = drawdown.index.astype(str)
        ax2.bar(time,drawdown.loc[:,0].values,width=0.5)
        ax2.set_title('Drawdown')
        ax2.grid(axis='y')
        ticks = [time[0+len(time)//10*i] for i in range(11)]
        ax1.set_xticks(ticks)
        ax1.set_xticklabels([i[:4] for i in ticks])
        ax2.set_xticks(ticks)
        ax2.set_xticklabels([i[:4] for i in ticks])
    ...
    def showResult(self):
        ...
        initial_cap = balance.values[0][0]
        final_cap = balance.values[-1][0]

        num = len(self.timeList)-self.advancedBar #number of time

        total_ret = final_cap/initial_cap - 1
        ret = (total_ret + 1)**(1/num)-1 #return
        ret_list = ((balance-balance.shift(1))/balance.shift(1)).dropna().values
        std = ret_list.std() #standard deviation
        negative_std = ret_list[ret_list<0].std() #negative standard deviation
        ...
        elif self.strategy.freq == 'D': #time unit: D
        ...
        print('Start Balance: $ '+ str(int(initial_cap)))
